...
`src/domain/teamFieldForms.ts`

- валидация формы поля команды;
- нормализация вариантов для `select` и `radio`;
- подготовка payload для form fields API.

`src/domain/teamFieldValues.ts`

- подготовка значений полей команды перед отправкой заявки;
- валидация обязательных visible team fields.
...
- содержит минимальный конструктор полей команды;
- позволяет создать поле команды, переключить `required/visible` и удалить поле;
...
- рендерит visible team fields из конструктора;
- валидирует required team fields;
- отправляет значения team fields в payload команды;
...
Для полей команды mock backend:

- принимает `GET /hackathons/:hackathonId/form-fields?scope=team`;
- принимает `POST /hackathons/:hackathonId/form-fields`;
...
- минимальный конструктор полей команды;
