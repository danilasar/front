...
impl HackathonRepository for HackathonRepo {
    ...
    {
        let id = Uuid::new_v4();
        let landing = req.landing.unwrap_or_else(|| crate::schemas::hackathons::HackathonLanding {
            hero_title: None,
            hero_subtitle: None,
            cover_file_id: None,
            content: None,
        ...
        .bind(landing.hero_title)
        .bind(landing.hero_subtitle)
        .bind(landing.cover_file_id)
        .bind(landing.content)
        .fetch_one(executer)
        .await
    }
